...
const __completeLineas = (genTable, data) => {
    ...
    genRow.addCell().addBlock(data.debe || '0')
    genRow.addCell().addBlock(data.haber || '0')//.setTextAlign('end')
    genRow.addCell().addBlock(data.concep || '')//.setTextAlign('end')
...
var rowC1 = checkTable.getBody().addRow()
imptotComplCount = 59 - mObjCheck.coment.length
rowC1.addCell().setColspan(7).addBlock(mObjCheck.coment + '*'.repeat(imptotComplCount))
...
linesTable.addColumn().setColumnWidth(2.2);
