...
const PAGE_LAST        = "FirstPage";
// ====================================================================
// CREATE DOCUMENT FOR CHECKS
// ====================================================================

var root = new Ax.fop.DocumentBuilder()
    // .addSimplePageMaster(PAGE_LAST, 0.5, 0.5, 0.5, 1.0)
    .createDocument(PAGE_WIDTH, PAGE_HEIGHT, 0, 0, 0, 0)
// root.setDebug("*")
root.getSimplePageMaster().setMargins(bandas, bandas, 0, 0)

root.getSimplePageMaster().getRegionBefore().setExtent(1);
...
const __completeLineas = (genTable, data) => {
    ...
    genRow.addCell().addBlock(data.orden || '')//.setMarginLeft(0.5)
    genRow.addCell().addBlock(data.docser || '')//.setTextAlign('center')
    genRow.addCell().addBlock(data.cuenta || '')//.setFontSize(7).setMarginLeft('-5pt')
    genRow.addCell().addBlock(data.nombre || '')
    genRow.addCell().addBlock(data.ctaaux || '')
    genRow.addCell().addBlock(data.desval || '')
    genRow.addCell().addBlock(data.debe || '0').setTextAlign('end')
    genRow.addCell().addBlock(data.haber || '0').setTextAlign('end')
    genRow.addCell().addBlock('')//.setTextAlign('end')
...
var mRsMain = Ax.db.executeQuery(`
        <select>
            <columns>
                TO_CHAR(cremesas.fecrem, '%d/%m/%Y') fecrem, cremesas.imptot, cefectos.codper, 
                CASE WHEN ctercero.nombre = 'PARTICULAR' THEN cremesas.coment
                     ELSE ctercero.nombre
                END nombre,
                CASE WHEN ctercero.nombre = 'PARTICULAR' THEN ''
                     ELSE cremesas.coment
                END coment,
                cbancpro.codban, cefectos.cuenta, cefectos.docser, 
                TO_CHAR(cefectos.fecha, '%d/%m/%Y') fecha, TO_CHAR(cefectos.fecven, '%d/%m/%Y') fecven, cefectos.impdiv, cbancpro.moneda,
                cefectos.jusser, cefectos.numero,
                ...
                (SELECT MAX(g.pcs_loteid)
                    FROM cefecges_pcs g, cefecges_det d
                    WHERE g.pcs_seqno   = d.pcs_seqno
                        AND g.date_created = (SELECT MIN(g2.date_created)
                                                FROM cefecges_pcs g2, cefecges_det d
                                                WHERE g2.pcs_seqno   = d.pcs_seqno
                                                AND d.det_numero  = cefectos.numero)
                        AND d.det_numero  = cefectos.numero) loteid
            </columns>
            <from table='cremesas'>
                <join table='cbancpro'>
...
mRsMain.cursor().forEach(row => {
    console.log(row.fecrem);
    let mStrFecemi = row.fecrem;
    let mStrFecven = row.fecven;
    let mStrFecha = row.fecrem;
    ...
    // mFloatDebAcum += row.debito || 0
    // mFloatCredAcum += row.credit || 0
    mIntLoteid = row.loteid;
})
...
linesTable.addColumn().setColumnWidth(0.4);
linesTable.addColumn().setColumnWidth(2.8);
linesTable.addColumn().setColumnWidth(3.7);
linesTable.addColumn().setColumnWidth(3.7);
 ...
 */
var mArrApuntes = Ax.db.executeQuery(`
            <select>
                    <columns>
                        capuntes.apteid, capuntes.fecha,  capuntes.asient, capuntes.orden,  
                        capuntes.docser, capuntes.diario, cdiarios.nomdia, 
                        capuntes.cuenta, ccuentas.nombre, capuntes.codaux, capuntes.ctaaux,
                        cctaauxl.desval, 
                        capuntes.concep, capuntes.debe,   capuntes.haber,  capuntes.divdeb, capuntes.divhab,
                        capuntes.moneda, cmonedas.desmon, capuntes.cambio, capuntes.fecval, capuntes.contra,
                        ccuentas2.nombre nomtra,
                        capuntes.loteid,
                        capuntes.ejerci,  capuntes.period
                    </columns>
                    <from table="capuntes">
                        ...
                        <join type="left" table="cmonedas">
                            <on>capuntes.moneda = cmonedas.codigo</on>
                        </join>
...
mArrApuntes.forEach(row => {
    __completeLineas(linesTable, row)
    mFloatDebAcum += row.debe
    mFloatCredAcum += row.haber
})
...
// var after = flow.addTable()

var totalTable = flow.addTable()

totalTable.setMarginTop(1).setFontFamily('monospace').setFontSize(8)
...
// root.getPageMaster(PAGE_LAST).getRegionBody().setColumnCount(2).setColumnGap(0.5);
// root.getPageMaster(PAGE_LAST).getRegionAfter().setExtent(2.0);
