    ...
    var rs = Ax.db.executeQuery(`
        <select>
            <columns>
                ...
                MAX(CASE 
                        WHEN ctax_move_head.taxh_natfac = 'C' AND ctax_move_head.taxh_tipdoc != 91 THEN '8.1'
                        WHEN ctax_move_head.taxh_natfac = 'C' AND ctax_move_head.taxh_tipdoc  = 91 THEN '8.2'
                        ELSE ''
                END)                                libro_ple,
                ...
                SUM(CASE
                        ...
                        WHEN ((ctax_move_line.taxl_code IN ('IGVG', 'IGVH') AND grp_ctax_prorrata.pror_seccio IS NULL) OR
                                ctax_move_line.taxl_code IN ('IGVS')) 
                        THEN NVL(ctax_move_line.taxl_basimp, 0) + NVL(ctax_move_line.taxl_basnimp, 0)
                        ELSE 0
                END) column_a1,
                ...
                SUM(CASE
                        WHEN ctax_move_head.taxh_natfac = 'V' AND ctax_move_line.taxl_code IN ('IGVG', 'IGVH') 
                        THEN 0
                        WHEN ctax_move_line.taxl_code IN ('IGVG', 'IGVH') AND grp_ctax_prorrata.pror_seccio IS NOT NULL 
                        THEN NVL(ctax_move_line.taxl_basimp, 0) + NVL(ctax_move_line.taxl_basnimp, 0)
                        ELSE 0
                END) column_b1,
                ...
                MAX(
                CASE
                    WHEN SUBSTR(ctax_move_head.taxh_docrec, 1, 1) IN ('E', 'F') 
                    THEN '01'   <!-- Factura origen -->
                    WHEN SUBSTR(ctax_move_head.taxh_docrec, 1, 1) = 'B' 
