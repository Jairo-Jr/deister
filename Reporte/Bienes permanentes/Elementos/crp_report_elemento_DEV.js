let mSqlCond = `cinmcomp.codele = '125012499'`;
...
Ax.db.execute(`
        <select intotemp = '${mTmpTableRsComp}'>
            <columns>
                ...
                NVL(CASE WHEN YEAR(cinmcomp.fecbaj) = YEAR(TODAY) AND cinmcomp.tipcom = 'B' THEN ABS(cinmcval.inicom)
                     WHEN YEAR(cinmcomp.fecha) &lt; YEAR(TODAY) THEN cinmcval.invcom
                     ELSE 0
                     END, 0)                                                                <alias name='crp_saldo_inicial' />,
                NVL(CASE WHEN YEAR(cinmcomp.fecha) = YEAR(TODAY)
                        AND cinmcomp.tipcom != 'J'
                        AND cinmcomp.docser NOT LIKE 'FINV%' THEN cinmcval.invcom
                     ELSE 0
                    END, 0)                                                                 <alias name='crp_imp_adqui' />,
                NVL(CASE WHEN cinmcomp.tipcom = 'M' THEN cinmcval.invcom
                    ELSE 0
                    END, 0)                                                                 <alias name='imp_mejoras'/>,
                NVL(CASE WHEN cinmcomp.tipcom = 'B' THEN ABS(cinmcval.inicom) * -1
                    ELSE 0   
                END, 0)                                                                     <alias name='imp_ret_baj' />,
                NVL(CASE WHEN cinmcomp.tipcom = 'J' THEN cinmcval.invcom
                     WHEN cinmcomp.docser LIKE 'FINV%' THEN cinmcval.invcom
                    ELSE 0   
                END, 0)                                                                     <alias name='imp_otros_ajus' />,
                NVL(cinmcval.inicom, 0)                                                         <alias name='imp_dep_acum_ejerc' />,
                NVL(CASE WHEN cinmcomp.tipcom = 'B' THEN ABS(cinmcval.inicom) * -1
                    ELSE 0   
...
var rsComponentes = Ax.db.executeQuery(`
        <select>
            <columns>
                ...
                MAX(nomsec),
                MAX(crp_cuenta_inmo),
                MAX(crp_cuenta_noinmo),
                MAX(crp_cuenta_chv),
                MAX(crp_inversion),
                MAX(crp_descri_inversion),
                MAX(crp_partida),
                MAX(crp_descri_partida),
    
                MAX(nro_contrato_arr),
                MAX(fecha_contrato_arr),
                SUM(nro_cuotas_arr),
                SUM(monto_total_arr),
                MAX(libro_af),
                SUM(crp_asiento_axional),
                MAX(crp_asiento_chavin),
                SUM(crp_saldo_inicial),
                SUM(crp_imp_adqui),
                SUM(imp_mejoras),
                SUM(imp_ret_baj),
                SUM(imp_otros_ajus),
                SUM(imp_dep_acum_ejerc),
                SUM(imp_dep_ret_baj),
                SUM(imp_dep_otr_ajus),
                
                SUM( ((crp_saldo_inicial+crp_imp_adqui+imp_mejoras+imp_ret_baj+imp_otros_ajus)
                     - (imp_dep_acum_ejerc+dep_39+imp_dep_ret_baj+imp_dep_otr_ajus)) ) neto,
                 
                MAX(cod_quiron),
                MAX(nro_aut_obra_quiron),
                MAX(estado),
                MAX(des_rub_cuenta),
                SUM(dep_39),
                SUM(dep_68),
                SUM(deprec_trib_anual),
                SUM(deprec_trib_acum),
                SUM(deprec_trib_baja),
                MAX(inventariable),
                MAX(grp_inmovilizado),
