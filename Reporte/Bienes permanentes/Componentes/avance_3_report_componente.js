...
// let mTmpTableCinmamorAcumEjerci = Ax.db.getTempTableName(`tmp_cinmamor_acum_ejerci`);
// Ax.db.execute(`DROP TABLE IF EXISTS ${mTmpTableCinmamorAcumEjerci}`);
// Ax.db.execute(`
//         <select intotemp='${mTmpTableCinmamorAcumEjerci}'>
//             <columns>
//                 cinmcomp.empcode,
//                 cinmcomp.codinm,
//                 cinmcomp.codele,
...
Ax.db.execute(`
    <select intotemp = '${mTmpTableRsComp}'>
        <columns>
            ...
            CASE WHEN cinmcomp.tipcom != 'B' 
                            THEN (NVL(cinmcval.inicom, 0) + NVL(${mTmpTableCinmamorEjercicio}.import, 0) + (NVL(CASE WHEN cinmcomp.tipcom = 'B' AND YEAR(cinmcomp.fecbaj) &lt; ${mIntYear} 
                                                                                                                            THEN ABS(cinmcval.inicom) * -1
                                                                                                                     ELSE 0 END, 0)))
                    ELSE 0
            ...
            (cinmcomp.auxnum5 + (CASE WHEN NVL(cinmcomp.auxchr3, -1) = -1 THEN NVL(${mTmpTableCinmamorEjercicio}.import_depre, 0)
                    ELSE NVL(cinmcomp.auxchr3, 0) + NVL(${mTmpTableCinmamorxConta_Abril}.import_depre, 0)
            END))                                                        <alias name='deprec_trib_acum' />,
