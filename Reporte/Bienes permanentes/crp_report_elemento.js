    ...
    let rsElementos = Ax.db.executeQuery(`
        <select>
            <columns>
                ...
                gdeparta.nomdep,
                cinmcomp.codcom                                              <alias name='crp_componente' />,
                cinmcomp.nomcom                                              <alias name='crp_description_componente' />,
                ...
                CASE WHEN YEAR(cinmcomp.fecbaj) = YEAR(TODAY) AND cinmcomp.tipcom = 'B' THEN ABS(cinmeval.iniele)
                        WHEN YEAR(cinmcomp.fecha) &lt; YEAR(TODAY) THEN cinmeval.invele
                        END                                                                <alias name='crp_saldo_inicial' />, 
                CASE WHEN YEAR(cinmcomp.fecha) = YEAR(TODAY)
                        AND cinmcomp.tipcom != 'J'
                        AND cinmcomp.docser NOT LIKE 'FINV%' THEN cinmeval.invele
                    END                                                                 <alias name='crp_imp_adqui' />,
                CASE WHEN cinmcomp.tipcom = 'M' THEN cinmeval.invele
                    ELSE 0
                    END                                                                 <alias name='Imp_Mejoras'/>,
                CASE WHEN cinmcomp.tipcom = 'B' THEN ABS(cinmeval.iniele) * -1
                    ELSE 0   
                END                                                                     <alias name='Imp_Ret_Baj' />,
                CASE WHEN cinmcomp.tipcom = 'J' THEN cinmeval.invele
                        WHEN cinmcomp.docser LIKE 'FINV%' THEN cinmeval.iniele
                    ELSE 0   
