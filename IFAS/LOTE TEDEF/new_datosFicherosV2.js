function datosFicherosV2(pdata, pIntIndicador) {
	// Protección ante algún error, realizamos un rollback
	Ax.db.commitWork();
	Ax.db.beginWork();
	
	let mUser 			= Ax.db.getUser();
	let mFecha 			= new Ax.util.Date().format("yyyy-MM-dd HH:mm:ss.SSS");
	...
	*/
		...
		<select>
			...
			<from table = 'fas_tedef_factura'>
				<join table = 'fas_factura_venta'>
					...
					<on>fas_tedef_factura.tdf_centro  = fas_factura_venta.fvh_centro</on>
        ...
        */
		...
		mRsDatosDfarm.forEach((mrow) => {
			try {
                ...
                if(contador2 == 0){
                    arrFacturas.push({"fvh_numero" : mrow.fvh_numero});
                    contador2 ++;
                    mStrFactura = mrow.fvh_numero
                }else if (mrow.fvh_numero != mStrFactura){
                    arrFacturas.push({"fvh_numero" : mrow.fvh_numero});
                    mStrFactura = mrow.fvh_numero;
                }
		...
		for (let mCountDate = 1; mCountDate <= mIntMax; mCountDate++) {
            console.log(mSqlMedico);
            console.log(mStrSqlCond);
            console.log(mUser);
            console.log(mFecha);
            console.log(mRowData.tdf_nrolote, mRowData.fvh_numero, mRowData.liq_episodio, mRowData.liq_cnt_id);
			...
			if (mRsDate.getRowCount() > 0) {
				...
				mRsDate.forEach((row) => {
					try {
                        ...
                        if(contador2 == 0){
                        arrFacturas.push({"fvh_numero" : row.fvh_numero});
                            contador2 ++;
                            mStrFactura = row.fvh_numero
                        }else if (row.fvh_numero != mStrFactura){
                            arrFacturas.push({"fvh_numero" : row.fvh_numero});
                            mStrFactura = row.fvh_numero
                        }
					}
    ...
    // console.log(mRsDatosDfacFichero);
	...
	mRsDatosDfacFichero.forEach((mRowData) => {
		...
		try {
			 ...
			 */
            // console.log('XD_1', mObjDate.tdate_totgstcub);
			...
			Ax.db.insert('fas_tedef_dfac_test',
				{
					...
					"tdfac_mtvnota"			: '',
					"tdfac_fecenv"			: '',
					"tdfac_indglob"         : 'N'        // mObjDfac.fvh_indicador (Indicador temporal)
				}
			);
		} catch(e) {
            console.log('DFAC', e)
            ...
            //if(contador2 == 0){
            //    arrFacturas.push({"fvh_numero" : mObjDfac.fvh_numero});
            //    contador2 ++;
            //    mStrFactura = mObjDfac.fvh_numero;
            //}else if (mObjDfac.fvh_numero != mStrFactura){
            //    arrFacturas.push({"fvh_numero" : mObjDfac.fvh_numero});
            //    mStrFactura = mObjDfac.fvh_numero;
            //}
		}
	...
	if (mStrFacturas != '') {
        ...
        let mIntSerial	= Ax.db.insert('fas_tedef_lote',
        	{
        		"tdl_nrolote"			: null,
        		"tdl_estado"			: 'P',
        		"tdl_situacion"			: '2',
                "tdl_env_cob_date"      : pdata.tdl_env_cob_date,
				"tdl_rec_fic_date"      : pdata.tdl_rec_fic_date,
        		"tdl_ind_lote_manual"	: '1',
                "tdl_nrolote_origen"    : mStrLote,
        		"user_created"			: Ax.ext.user.getCode(),
        		"user_updated"			: Ax.ext.user.getCode()
        	}
        ).getSerial();

        let mStrCodigo = Ax.db.executeGet(`
        	<select>
        		<columns>
        			tdl_nrolote
        		</columns>
        ...
        arrFacturas.forEach(row1 => {
            let row = Ax.db.executeQuery(`
            	<select>
            		<columns>
            			tdf_factura,
                        tdf_nrolote,
            ...
            `,row1.fvh_numero, mStrLote ).toOne();

        	mStrNrolote = row.tdf_nrolote;

        	Ax.db.update('fas_tedef_factura',
        		{
        			"tdf_nrolote"		: mStrCodigo,
        			"user_updated"		: Ax.ext.user.getCode(),
        			"date_updated"		: new Ax.util.Date()
        		},
        		{
        			"tdf_factura"		: row.tdf_factura,
        			"tdf_nrolote"		: mStrNrolote
        		}
        	);

        	Ax.db.update('fas_tedef_lote',
        		{
        			"tdl_ambito"		: row.tdl_ambito,
        			"tdl_financiador"	: row.tdl_financiador,
        			"tdl_meca_pago"		: row.tdl_meca_pago,
        			"tdl_tipo_comprob"	: row.tdl_tipo_comprob,
        			"user_updated"		: Ax.ext.user.getCode(),
        			"date_updated"		: new Ax.util.Date()
        		},
        		{
        			"tdl_nrolote"		: mStrCodigo
        		}
        	);
        });

        Ax.db.call('actualizaLote',mStrCodigo);
        Ax.db.call('actualizaLote',mStrNrolote);
        Ax.db.call('eliminaLote');
    }
...
var pdata = {
    tdl_nrolote: '0618917',
    tdf_centro: 'CRP0',
    tdl_financiador: '00058743'
}

// var pdata = {
//     tdl_nrolote: '0622736',
//     tdf_centro: 'CRP0',
//     tdl_financiador: '00042950'
// }

datosFicherosV2(pdata, 1)