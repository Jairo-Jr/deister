	...
	let mRsInicial = Ax.db.executeQuery(`
		<select>
			<columns>
				...
				CASE WHEN ${mIndFinanciador} = 0 THEN adm_episodio_padre
					 ELSE NULL
				END adm_episodio_padre
			</columns>
				...
				mRsDate.forEach((row) => {
					try {
						...
						// Si es RIMAC, se aplican las siguientes reglas
							...
							if (mObjCartaGarantia) {
								mStrTipoDoc	= mArrCartaGarantia.acg_origen;
								mStrNroDoc	= mObjCartaGarantia;
							}
						}
			...
			Ax.db.insert('fas_tedef_dfac',
				{
					...
					"tdfac_mtvnota"			: '',
					"tdfac_fecenv"			: '',
					"tdfac_indglob"			: 'N'		// mObjDfac.fvh_indicador
				}
			);
		} catch(e) {
			/**
