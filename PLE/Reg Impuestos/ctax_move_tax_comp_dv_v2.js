let mCondInput = `ctax_move_head.taxh_docser = 'E001-00000007' AND ctax_move_head.taxh_natfac = 'C'`;
    var mEJEINI    = 2024;
    var mEJEFIN    = 2024;
    var mPERINI    = 7;
    var mPERFIN    = 7;
    var mFECHA_INI = `'19-07-2024'`;
    var mFECHA_FIN = `'19-07-2024'`;
    ...
    Ax.db.execute(`
        <select intotemp='${mTmpGrpCtaxDetEfec}'>
            <columns>
                ...
                MAX(CASE WHEN NVL(ccomfach.tipdoc, '-1') != '-1' THEN ccomfach.tipdoc
                         WHEN NVL(cvenfach.tipdoc, '-1') != '-1' THEN cvenfach.tipdoc
                         WHEN NVL(gcomfach.tipdoc, '-1') != '-1' THEN gcomfach.tipdoc
                         WHEN NVL(gvenfach.tipdoc, '-1') != '-1' THEN gvenfach.tipdoc
                    END) fact_tipdoc,

                MAX(CASE WHEN NVL(ccomfach.dockey, '-1') != '-1' THEN ccomfach.dockey
    ...
    var rs = Ax.db.executeQuery(`
        <select>
            <columns>
                ...
                MAX(CASE WHEN grp_ctax_det_efec.fact_tipdoc = 'NRHM'
                              AND grp_ctax_det_efec.fact_dockey = '02' THEN '07'
                         WHEN grp_ctax_det_efec.fact_tipdoc = 'NRHM'
                              AND grp_ctax_det_efec.fact_dockey = '07' THEN '02'
                         ELSE ctax_move_head.taxh_tipdoc
                END)                                                                    taxh_tipdoc,   <!-- Tipo de documento -->
                MAX(CASE 
                ...
                END)                                libro_ple,
                MAX(CASE WHEN grp_ctax_det_efec.fact_tipdoc = 'NRHM'
                              AND grp_ctax_det_efec.fact_dockey = '02' THEN 'Nota de crédito'
                         WHEN grp_ctax_det_efec.fact_tipdoc = 'NRHM'
                              AND grp_ctax_det_efec.fact_dockey = '07' THEN 'Honorarios'
                         ELSE pe_sunat_tab10.tipo_cdp_desc
                    END)                                                                tipo_cdp_desc, <!-- Descripción tipo documento -->
