        ...
        <union type='all' intotemp='${mTmpTableActivos}'>
            ...
            <select>
                <columns>
                    ...
                    0 <alias name='imp_ret_baj' />,
                    0 <alias name='imp_otros_ajus' />,
                    MAX(cinmamor.fecfin) <alias name='fec_depre' />
                </columns>
                <from table='cinmhead'>
                    <join table='cinmelem'>
...
Ax.db.execute(`
            <select intotemp='${mTmpTableCinmelem}'>
                ...
                <from table='cinmhead'>
                    <join table='cinmelem'>
                        ...
                        <join type='left' table='${mTmpTableCinmelemPpe}' alias='cinmelem_ppe'>
                            <on>cinmelem.empcode = cinmelem_ppe.empcode</on>
                            <on>cinmelem.codinm = cinmelem_ppe.codinm</on>
                            <on>cinmelem.codele = cinmelem_ppe.codele</on>
                        </join>
                ...
                <where>
                    ...
                    AND cinmelem.codinm NOT IN ('999999', '777777')
