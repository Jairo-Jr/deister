...
// throw typeof mIntYear + '-' + typeof mIntMonth;
// throw  mDateTimeIniPeriodoInf + '-' +mDateTimeFinPeriodoInf;
            ...
            <columns>
                ...
                CAST(
                    CASE WHEN CHARINDEX('-', capuntes.docser) &gt; 0 THEN REPLACE(TRIM(SUBSTRING_INDEX(capuntes.docser, '-', 1)), '/', '')
                        WHEN NVL(tmp_capuntes.tipdoc, '00') = '46' THEN '100'
                        ELSE ''
                    END
                    AS CHAR(8)
                ...
                CAST(
                    REPLACE(REPLACE(CASE WHEN LENGTH(SUBSTRING_INDEX(capuntes.docser, '-', -1)) > 0 THEN SUBSTRING_INDEX(capuntes.docser, '-', -1)
...
if (mIntNumCapuntes % 2 == 0){
    var mIntCantGr1 = (mIntNumCapuntes / 2).toString();
    var mIntCantGr2 = (mIntNumCapuntes / 2).toString();
} else {
    var mIntCantGr1 = Math.floor(mIntNumCapuntes / 2);
    var mIntCantGr2 = mIntCantGr1 + 1;

    mIntCantGr1 = mIntCantGr1.toString()
    mIntCantGr2 = mIntCantGr2.toString();
}
console.log(mIntNumCapuntes);
console.log(mIntCantGr1, mIntCantGr2);
...
if(pStrCondicion == 'F') {
    ...
    var mRsPle_5_1_pt1 = __getRSCapuntes({
        first_str: `first = '${mIntCantGr1}'`,
        columns: 'capuntes.apteid,',
        periodo: mStrCodPeriodo,
        tbl_tmp: mTmpTableGrpCapuntes,
        fec_ini: mDateTimeIniPeriodoInf,
        fec_fin: mDateTimeFinPeriodoInf,
        order_str: '<order>capuntes.apteid ASC</order>'
    });

    var blob_pt1 = new Ax.sql.Blob(`ple_0501_${mIntYear}_${mIntMonth}_part1.csv`);
    ...
    // Definición de file zip
    // ===============================================================
    var ficherozip_pt1 = new Ax.io.File("/tmp/ziptest.zip");
    ...
    var mRsPle_5_1_pt2 = __getRSCapuntes({
        first_str: `first = '${mIntCantGr2}'`,
        columns: 'capuntes.apteid,',
        periodo: mStrCodPeriodo,
        tbl_tmp: mTmpTableGrpCapuntes,
        fec_ini: mDateTimeIniPeriodoInf,
        fec_fin: mDateTimeFinPeriodoInf,
        order_str: '<order>capuntes.apteid DESC</order>'
    });
    mRsPle_5_1_pt2 = mRsPle_5_1_pt2.rows().sort('apteid');

    var blob_pt2 = new Ax.sql.Blob(`ple_0501_${mIntYear}_${mIntMonth}_part2.csv`);
    ...
    // Definición de file zip
    // ===============================================================
    var ficherozip_pt2 = new Ax.io.File("/tmp/ziptest.zip");
    ...
    mRsFile.rows().add([`ple_0501_${mIntYear}_${mIntMonth}_part1.csv`, fichero_pt1.getBytes()]);
    mRsFile.rows().add([`ple_0501_${mIntYear}_${mIntMonth}_part2.csv`, fichero_pt2.getBytes()]);
