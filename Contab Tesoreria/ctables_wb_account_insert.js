...
function ctables_wb_account_insert(p_tempTables, p_tabname, p_facidx, p_regid) {
    ...
    function local_load_capuntes(l_tmp_capuntes, l_tabname, l_gencon) {
        ...
        // cambia debe y haber.
        // ========================================================================
        console.log('DESCON', tmp_capuntes.descon);
        console.log('TMP', tmp_capuntes);
        ...
        // ========================================================================
        // [28-12-2023]
        // Distinción de registros por indicador de agrupa
        // ========================================================================
        var mIntRegTmp = Ax.db.executeQuery(`SELECT SUM(CASE WHEN agrupa = 'N' THEN 1 ELSE 0 END) num_n,
		                               SUM(CASE WHEN agrupa = 'S' THEN 1 ELSE 0 END) num_s 
		                        FROM ${l_tmp_capuntes}`).toOne();

        // ========================================================================
        // Para registros con indicador, agrupa = 'N'
        // ========================================================================
        if(mIntRegTmp.num_n > 0){
            // ========================================================================
            // Insert journal (capuntes) from non-grouping lines.
            // ========================================================================

            /**
             * Obtencion de la moneda de la cuenta financiera
             */
            var mStrBancMoneda = Ax.db.executeGet(`
    		    <select first='1'>
                    <columns>
                        cbancpro.moneda
                    </columns>
            ...
            /**
             * CRP [27-02-2024]
             *  Por el caso de pago de USD en cuenta PEN:
             *   - Se compara que si las monedas del apuntes y el banco corresponde con la de la empresa (PEN),
             *     el importe del divdeb y divhab toma el importe del deb y haber respectivamente
             */
            var sqlca = Ax.db.execute(`
    			...
    			SELECT  0,        CAST(? AS INTEGER), CAST(? AS INTEGER),
    					...
    					CASE WHEN moneda = 'PEN' AND '${mStrBancMoneda}' = 'PEN' THEN ROUND(debe,   ?)
    					     ELSE ROUND(divdeb, ?)
    					END divdeb,
    					
    					CASE WHEN moneda = 'PEN' AND '${mStrBancMoneda}' = 'PEN' THEN ROUND(haber,  ?)
    					     ELSE ROUND(divhab, ?)
    					END divhab
    			  FROM ${l_tmp_capuntes}
    			 WHERE agrupa = "N"
        ...
        // ========================================================================
        // Para registros con indicador, agrupa = 'S'
        // ========================================================================
        if(mIntRegTmp.num_s > 0){
            ...
            /* console.log(
                Ax.db.executeQuery(`
                    SELECT 0,      CAST(? AS INTEGER), CAST(? AS INTEGER),
    				   ...
    				   ROUND(SUM(debe),  ?) debe,
    				   ROUND(SUM(haber),   ?) haber,
    				   ROUND(SUM(divdeb), ?) divdeb,
    				   ROUND(SUM(divhab), ?) divhab
    			  FROM ${l_tmp_capuntes}
    			 WHERE agrupa = 'S'
        ...
        // ========================================================================
        // Si no existen registros en la temporal con indicador de agrupa 'N' o 'S'
        // ========================================================================
        if (mIntRegTmp.num_n == 0 && mIntRegTmp.num_s == 0){
            throw new Error("No existe registros para insertar en capuntes");
        }
    ...
    function local_load_ctax_move(l_tmp_ctax_move_head, l_tmp_ctax_move_line, l_tabname, l_apteid, l_loteid, l_asient,  l_facidx) {
        ...
        let mStrColOri = l_tabname == "cvenfach"
            ? `taxh_auxchr1, taxh_auxchr2, `
            : "";

        let mStrColDes = l_tabname == "cvenfach"
            ? `taxh_auxchr1, taxh_auxchr2, `
            : "";
    ...
    function local_load_cefectos(l_tmp_cefectos, l_tabname, l_apteid) {
        ...
        let mStrColOri = l_tabname == "cvenfach"
            ? `auxchr1, auxchr2, auxchr3, auxchr4, auxchr5, auxnum2, auxnum3, auxnum5, `
            : "coment, ";

        let mStrColDes = l_tabname == "cvenfach"
            ? `t.auxchr1, t.auxchr2, t.auxchr3, t.auxchr4, t.auxchr5, t.auxnum2, t.auxnum3, t.auxnum5, `
            : "t.coment, ";
    ...
    console.log(
        Ax.db.executeQuery(`SELECT SUM(divdeb) divdeb, SUM(divhab) divhab, SUM(debe) debe, SUM(haber) haber FROM ${p_tempTables.capuntes}`)
    );

    /* console.log(
        Ax.db.executeQuery(`SELECT divdeb, divhab, debe, haber FROM ${p_tempTables.capuntes}`)
    ); */
     ...
     * CRP [26/10/2023]
     *
     * Generalizacion de proceso
     */
    if(capuntes.loteid != null) {
        var mRSApteids = Ax.db.executeQuery(`SELECT apteid FROM capuntes WHERE loteid = ? AND cuenta LIKE '6%'`, capuntes.loteid).toJSONArray();

        for(let row of mRSApteids) {
            Ax.db.executeProcedure("capuprog_genera", row.apteid).toValue();
        }
    }
