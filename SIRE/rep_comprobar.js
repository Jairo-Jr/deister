let mCondInput = `ctax_move_head.taxh_ejefis = 2024 AND ctax_move_head.taxh_perfis = 1 AND ctax_move_head.taxh_natfac = 'C'`;
var mEJEINI = 2024;
var mEJEFIN = 2024;
var mPERINI = 1;
var mPERFIN = 1;
var mFECHA_INI = `'01-01-2024'`;
var mFECHA_FIN = `'31-01-2024'`;

var mSqlCond = `${mCondInput} AND
                ctax_move_head.taxh_ejefis BETWEEN ${mEJEINI} AND ${mEJEFIN} AND
                ctax_move_head.taxh_perfis BETWEEN ${mPERINI} AND ${mPERFIN} AND
                ctax_move_head.taxh_fecha  BETWEEN ${mFECHA_INI} AND ${mFECHA_FIN}`;
...
/**
 * Agrupado de ctax_move_head para obtener uniformidad en documentos fiscales con mas de una linea
 */
...
Ax.db.execute(`
            <select intotemp='${mTmpGrpCtaxHead}'>
                <columns>
                    ...
                    MAX(ctax_move_head.taxh_import) taxh_import,
...
var rs = Ax.db.executeQuery(`
        <select>
            <columns>
                ...
                grp_ctax_head.taxl_oper,      <!--ctax_move_line.taxl_code,-->
                ...
                0 renta_1,
                0 precio_1,
                CASE WHEN (NVL(ctax_move_line.taxl_code, ctax_type.type_code) IN ('IGVG', 'IGVH') AND grp_ctax_prorrata.pror_seccio IS NULL) OR
                            (NVL(ctax_move_line.taxl_code, ctax_type.type_code) IN ('IGVS')) THEN 'A'
                        WHEN NVL(ctax_move_line.taxl_code, ctax_type.type_code) IN ('IGVG', 'IGVH') AND grp_ctax_prorrata.pror_seccio IS NOT NULL THEN 'B'
                        WHEN NVL(ctax_move_line.taxl_code, ctax_type.type_code) IN ('IGVE', 'IGVI', 'IGVX') THEN 'D'
                        WHEN NVL(ctax_move_line.taxl_code, ctax_type.type_code) IN ('R4C', 'R4C0') THEN 'E'
            ...
            <from table='ctax_move_head'>
                <join table='ctax_type'>
                    <join table="ctax_class">
                        ...
                        <on>ctax_class.class_type IN ('N', 'R')</on>
                        <on>ctax_type.type_class IN ('IGV', 'RET')</on>
                    </join>
            ...
            <where>
                ctax_type.type_code IN ('IGVG', 'IGVE', 'IGVX', 'IGVI', 'IGVH', 'IGVS', 'R4C', 'R4C0') AND
                ctax_move_line.taxl_type IN ('N', 'R') AND
                cefectos.apteid = ctax_move_head.taxh_apteid AND
                ${mSqlCond}
                AND ctax_move_head.taxh_tipdoc = '01'
            </where>
            <order>
                ctax_move_head.taxh_seqno, ctax_move_head.taxh_apteid, 56
            </order>
        </select>
    `);
...
for (let mRowPivot of mRsPivot) {
    mRowPivot.igv_1 = (mRowPivot.c1 || 0) + (mRowPivot.c2 || 0);
    mRowPivot.renta_1 = (mRowPivot.c3 || 0) + (mRowPivot.c4 || 0);
    mRowPivot.precio_1 = (mRowPivot.c1 || 0) + (mRowPivot.c2 || 0) + (mRowPivot.c3 || 0) + (mRowPivot.c4 || 0);

    mRowPivot.igv_2 = (mRowPivot.c5 || 0) + (mRowPivot.c6 || 0);
    mRowPivot.renta_2 = (mRowPivot.c7 || 0) + (mRowPivot.c8 || 0);
    mRowPivot.precio_2 = (mRowPivot.c5 || 0) + (mRowPivot.c6 || 0) + (mRowPivot.c7 || 0) + (mRowPivot.c8 || 0);
